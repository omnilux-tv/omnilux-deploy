...
- `.github/workflows/desktop-installers.yml` publishes launch-facing desktop installer artifacts named `OmniLux-macOS.pkg`, `OmniLux-Linux.AppImage`, and `OmniLux-Setup.exe` to this repo's release page, plus `.sha256` checksum sidecars for each artifact. The default launch path requires Developer ID Installer signing and notarization secrets for macOS and an Authenticode code-signing certificate for Windows. The workflow has an explicit `allow_unsigned_artifacts` escape hatch for non-launch validation runs only.
